...
int find_symtab(uint8_t *file, const Elf64_Ehdr *eHdr, const char *shStrTab_data) {
    ...
    for (int i = 0; i < eHdr->e_shnum; i++) {
        const Elf64_Shdr *sHdr = (Elf64_Shdr *)(file + eHdr->e_shoff + (sizeof(Elf64_Shdr) * i));
...
bool print_all_symbols(uint8_t *file, const Elf64_Ehdr *eHdr, int symtabNdx) {
    ...
    int strtab_offset = eHdr->e_shoff + symTab->sh_link * sizeof(Elf64_Shdr);
    if (strtab_offset >= file_size) {
        ft_printfd(1, "File too short\n");
        return 1;
    }

    const Elf64_Shdr *strTab_section = (Elf64_Shdr *)(file + strtab_offset);
